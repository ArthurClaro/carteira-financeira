<div class="rounded-xl border border-brand-100 bg-white p-6 shadow-sm transition hover:shadow-md">
    ...
    <form wire:submit="transfer" class="space-y-3">
        <div>
            ...
            @error('recipient_email') <p class="animate-slide-in mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
        </div>
        ...
        <button type="submit" wire:loading.attr="disabled" wire:target="transfer"
            ...
            <svg wire:loading wire:target="transfer" class="size-4 animate-spin" viewBox="0 0 24 24" fill="none" aria-hidden="true">
